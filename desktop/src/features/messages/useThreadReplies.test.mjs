...
// ── Runtime stubs ──────────────────────────────────────────────────────────────
// Stub Tauri and relay-backed aux hydration before the hook module loads.
...
registerHooks({
  resolve(specifier, context, nextResolve) {
    ...
    if (specifier === "@/features/messages/lib/auxBackfill") {
      return { shortCircuit: true, url: "buzz-thread-stub:aux-backfill" };
    }
    if (specifier === "@/shared/api/relayClient") {
      return { shortCircuit: true, url: "buzz-thread-stub:relay-client" };
    }
...
`,
    ...
    if (url === "buzz-thread-stub:aux-backfill") {
      return {
        format: "module",
        shortCircuit: true,
        source: `
export function collectMessageIdsForAuxBackfill(events) {
...
`,
    ...
    if (url === "buzz-thread-stub:relay-client") {
      return {
        format: "module",
        shortCircuit: true,
        source: `
...
test("thread aux hydration includes the root when there are no replies", async () => {
  const { collectThreadAuxMessageIds } = await import("./useThreadReplies.ts");
  assert.deepEqual(collectThreadAuxMessageIds(ROOT_ID, []), [ROOT_ID]);
});

test("thread aux hydration includes and deduplicates root and reply ids", async () => {
  const { collectThreadAuxMessageIds } = await import("./useThreadReplies.ts");
  assert.deepEqual(
