...
/** Lifecycle bookkeeping controls actions; it is not relay availability. */
...
export async function deleteManagedAgentWithRules({
  ...
  if (agent.backend.type === "provider" && agent.backendAgentId) {
    ...
    if (channelId) {
      // Unknown is not evidence that shutdown can safely be skipped. Only an
      // established Offline result preserves the intentional local-only path.
        ...
        if (!skipRemoteDeleteConfirm) {
          const confirmed = window.confirm(
            (availability === undefined
              ...
              "will be orphaned if shutdown has not completed. Continue?",
          );
          if (!confirmed) {
